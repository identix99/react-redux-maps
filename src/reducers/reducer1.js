...
const inhitialState = {
  ...
  const contactReducer = (state = inhitialState, action) => {
    switch (action.type) {
      ...
      case EditContacts :
        let MatchingTab = state.contacts.filter(contact=>contact.id == action.payload)

        MatchingTab = MatchingTab.values();

        for(let val of MatchingTab){
          MatchingTab = val ;
          // console.log(val);
        }
         return {
         ...state ,
        Fillcontacts : MatchingTab ,
      }


      case UpdateContacts:  return {
            ...state ,
            contacts:  state.contacts.map(con => con.id == action.payload.id ? action.payload : con )
      }

      case DeleteContacts : return {
        ...state ,
